...
const UserinfoPage: React.FC = () => {
  ...
  const fetchData = (values: any) => {
    ...
    request('/project/list', { method: 'POST', data: _formData }).then((res: any) => {
  ...
  const handleDelete = (item: Record<string, any>) => {
    request('/project/delete', { method: 'POST', data: { id: item.id } }).then((res: any) => {
      if (res?.code == 200) {
  ...
  return (
    <div>
      ...
      >
        ...
        <Column title="项目名称" dataIndex="name" width={200} />
        <Column title="管理者" align="center" dataIndex="user" width={100} />
        <Column
          title="所属分类"
          dataIndex="category"
          width={200}
