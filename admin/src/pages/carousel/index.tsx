import { Button, Form, Image, Input, Popconfirm, Space, Table, Tag, message } from 'antd';
...
const UserinfoPage: React.FC = () => {
  ...
  const fetchData = (values: any) => {
    ...
    request('/carousel/list', { method: 'POST', data: _formData }).then((res: any) => {
  ...
  const handleDelete = (item: Record<string, any>) => {
    request('/carousel/delete', { method: 'POST', data: { id: item.id } }).then((res: any) => {
      if (res?.code == 200) {
    ...
    <div>
      ...
      >
        <Column
          ...
          dataIndex="pic"
          width={240}
          render={(_value: any, record: Record<string, any>) => <Image height="120px" src={_value} />}
        />
        <Column title="名称" align="center" dataIndex="name" width={200} />
        <Column title="跳转地址" align="center" dataIndex="path" width={200} />
        <Column
          title="更新时间"
        ...
        <Column
          ...
          render={(_value: any, record: Record<string, any>) =>
            _value ? <Tag color="#87d068">启用</Tag> : <Tag color="#f50">禁用</Tag>
          }
        />
        <Column
          title="操作"
