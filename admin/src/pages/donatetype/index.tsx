...
const UserinfoPage: React.FC = () => {
  ...
  const fetchData = (values: any) => {
    ...
    request('/donatetype/list', { method: 'POST', data: _formData }).then((res: any) => {
  ...
  const handleDelete = (item: Record<string, any>) => {
    request('/donatetype/delete', { method: 'POST', data: { id: item.id } }).then((res: any) => {
      if (res?.code == 200) {
      ...
      >
        ...
        <Column
          ...
          render={(_value: any, record: Record<string, any>) =>
            _value ? <Tag color="#87d068">是</Tag> : <Tag color="#f50">否</Tag>
          }
        />
        <Column
          title="是否启用"
