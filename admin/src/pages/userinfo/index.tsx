import { Avatar, Button, Form, Input, Popconfirm, Space, Table, message } from 'antd';
...
const UserinfoPage: React.FC = () => {
  ...
  const fetchData = (values: any) => {
    ...
    request('/userinfo/list', { method: 'POST', data: _formData }).then((res: any) => {
  ...
  const handleDelete = (item: Record<string, any>) => {
    request('/userinfo/delete', { method: 'POST', data: { id: item.id } }).then((res: any) => {
      if (res?.code == 200) {
  ...
  return (
    <div>
      <Form layout="inline" onFinish={(val) => handleQuery({ ...queryParams, ...val })} autoComplete="off">
        <Form.Item label="用户名" name="username" rules={[{ required: false, message: '不能为空！' }]}>
          <Input allowClear />
        </Form.Item>
        <Form.Item>
      ...
      >
        <Column
          ...
          render={(_value: any, record: Record<string, any>) =>
            _value ? <Avatar src={_value} /> : <Avatar size={40}>{record.username}</Avatar>
          }
        />
        <Column title="用户名" align="center" dataIndex="username" width={200} />
        <Column title="openid" align="center" dataIndex="openid" width={200} />
        <Column
          title="是否是管理员"
          dataIndex="isAdmin"
          align="center"
          width={200}
          render={(_value: any, record: Record<string, any>) => (_value ? '是' : '否')}
        />
        <Column
          title="创建时间"
